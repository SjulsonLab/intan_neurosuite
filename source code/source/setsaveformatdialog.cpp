...
{ 
    ...
    QLabel *label2 = new QLabel(tr("This option creates a subdirectory and saves raw data files for each "
                                   "signal type.  Amplifier, analog, and auxiliary (i.e. accelerometer) inputs are saved in a "
                                   "single file called amplifier_analogin_auxiliary_int16.dat.  Other signals "
                                   "are saved separately, not necessarily in int16 format (which Neuroscope "
                                   "uses).  To convert the amplifier input to microvolts, multiply by 0.195."
                                   "To convert the analog inputs to volts, multiply by 0.00010071, and to "
                                   "convert auxiliary to volts, multiply by 0.0000748 (these are different scaling "
                                   "factors than in the original version of this software from Intan).  The "
    ...
    label2->setWordWrap(true); // text edited by Luke

    QLabel *label2b = new QLabel(tr("The amplifier_analogin_auxiliary_int16.dat file is compatible with Neuroscope."));
