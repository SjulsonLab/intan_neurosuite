// code edited by Luke Sjulson, 2018-02-15, to save amplifier, analogin,
// and auxiliary data streams in a single .dat file, all in int16 format.
// The analogin and auxiliary channels' values are divded by two, so the
// gain factor to convert to volts will be twice what the Intan documentation says.
//
//  ------------------------------------------------------------------------
//
...
//#include "qtincludes.h" - commented out by Luke
...
{
    amplifierFileName = path + "/" + "amplifier_analogin_auxiliary_int16" + ".dat"; // edited by Luke, 2/2018
    auxInputFileName = path + "/" + "auxiliary" + ".dat"; // should be an empty file
    supplyFileName = path + "/" + "supply" + ".dat";
    adcInputFileName = path + "/" + "analogin" + ".dat"; // leaving this one here - should be empty anyway
...
void SignalProcessor::openSignalTypeFiles(bool saveTtlOut)
{
    ...
    //if (saveListAmplifier.size() > 0) { // edited by Luke - needs to generate amplifierStream even if there are no amplifier channels
    ...
    for (block = 0; block < numBlocks; ++block) {
        ...
        if (saveToDisk) {
            ...
            case SaveFormatFilePerSignalType:
                ...
                // saving amplifier, ADC, and auxiliary channels together a single .dat file - edited by Luke, 2/2018
                bufferIndex = 0;
                for (t = 0; t < SAMPLES_PER_DATA_BLOCK; ++t) {
                    for (i = 0; i < saveListAmplifier.size(); ++i) { // write amplifier data to buffer
                        tempQint16 = (qint16)
                    ...
                    for (i = 0; i < saveListBoardAdc.size(); ++i) { // write ADC data to buffer
                        tempQint16 = (qint16)
                            (dataQueue.front().boardAdcData[saveListBoardAdc.at(i)->nativeChannelNumber][t] / 2); // edited by Luke - dividing by 2 to convert to qint16
                        dataStreamBuffer[bufferIndex++] = tempQint16 & 0x00ff;         // Save qint16 in little-endian format (LSByte first)
                        dataStreamBuffer[bufferIndex++] = (tempQint16 & 0xff00) >> 8;  // (MSByte last)
                    }
                    // writing auxiliary (accelerometer) input data to buffer
                    tAux = 4 * qFloor((double) t / 4.0);
                    for (i = 0; i < saveListAuxInput.size(); ++i) {
                        tempQint16 = (qint16)
                            (dataQueue.front().auxiliaryData[saveListAuxInput.at(i)->boardStream][1][tAux + saveListAuxInput.at(i)->chipChannel + 1] / 2); // edited by Luke - div by 2 for qint16
                        dataStreamBuffer[bufferIndex++] = tempQint16 & 0x00ff;         // Save qint16 in little-endian format (LSByte first)
                        dataStreamBuffer[bufferIndex++] = (tempQint16 & 0xff00) >> 8;  // (MSByte last)
                    }
                ...
                if (bufferIndex > 0) {
                    ...
                    numWordsWritten += (saveListAmplifier.size() + saveListBoardAdc.size() + saveListAuxInput.size()) * SAMPLES_PER_DATA_BLOCK;
                }
                                      ...
                                      bool saveTemp, bool saveTtlOut, int timestampOffset)
{
    ...
    case SaveFormatFilePerSignalType:
        while (bufferQueue.empty() == false) {
            ...
            // Save amplifier, ADC, and auxiliary data in single .dat file - edited by Luke, 2/2018
            bufferIndex = 0;
            for (t = 0; t < SAMPLES_PER_DATA_BLOCK; ++t) {
                for (i = 0; i < saveListAmplifier.size(); ++i) { // write amplifier data to buffer
                    tempQint16 = (qint16)
                ...
                for (i = 0; i < saveListBoardAdc.size(); ++i) { // write ADC data to buffer, converting to int16 format first
                    tempQint16 = (qint16)
                        (bufferQueue.front().boardAdcData[saveListBoardAdc.at(i)->nativeChannelNumber][t] / 2);
                    dataStreamBuffer[bufferIndex++] = tempQint16 & 0x00ff;         // Save qint16 in little-endian format (LSByte first)
                    dataStreamBuffer[bufferIndex++] = (tempQint16 & 0xff00) >> 8;  // (MSByte last)
                }

                tAux = 4 * qFloor((double) t / 4.0); // writing auxiliary to buffer, converting to int16 format first - edited by Luke
                for (i = 0; i < saveListAuxInput.size(); ++i) {
                    tempQint16 = (qint16)
                        (bufferQueue.front().auxiliaryData[saveListAuxInput.at(i)->boardStream][1][tAux + saveListAuxInput.at(i)->chipChannel + 1] / 2);
                    dataStreamBuffer[bufferIndex++] = tempQint16 & 0x00ff;         // Save qint16 in little-endian format (LSByte first)
                    dataStreamBuffer[bufferIndex++] = (tempQint16 & 0xff00) >> 8;  // (MSByte last)
                }
        ...
        case SaveFormatFilePerSignalType:
            for (block = 0; block < numBlocks; ++block) {
                ...
                for (t = 0; t < SAMPLES_PER_DATA_BLOCK; ++t) {
                    ...
                    // Save auxiliary input data - edited by Luke to be qint16 format and save to amplifierStream, 2018/2
                    tAux = qFloor((double) t / 4.0);
                    for (i = 0; i < saveListAuxInput.size(); ++i) {
                        *(amplifierStream) << (qint16)
                            (auxChannel[saveListAuxInput.at(i)->boardStream][saveListAuxInput.at(i)->chipChannel][15 * block + tAux] / 0.0000748); // scaling factor edited by Luke
                        ++numWordsWritten;
                    }
                    // Save supply voltage data
                    for (i = 0; i < saveListSupplyVoltage.size(); ++i) {
                    ...
                    // Save board ADC data - edited by Luke to be qint16 format and save to amplifierStream, 9/2016
                    for (i = 0; i < saveListBoardAdc.size(); ++i) {
                        *(amplifierStream) << (qint16) 0;
                        ++numWordsWritten;
                    }
                    // Save board digital input data
                    if (saveListBoardDigIn) {
